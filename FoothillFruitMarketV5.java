...
class Fruit{
	...
	public boolean equals(Object other) {
        ...
        Fruit comparedFruit  = (Fruit) other;  
        return (fruitName == comparedFruit.fruitName && weightAvailable == comparedFruit.weightAvailable && pricePerLb == comparedFruit.pricePerLb);
	}
...
class OnlineSuperMarket{
	...
	public void init() {
		...
		double [] totalWeight = {16,      24,       39,       14,           55,     16,      27,      8.5,        19,            10};
		double [] priceArr    = {2,       1.35,     4.3,      2.5,          3.4,    8.2,     3.4,     2.2,        1.8,           5.4};
	...
	public void run() {
		...
		do {
			...
			if (choice.equals("Q") || choice.equals("q")) {
				...
				Fruit foundFruit = find(choice);
				if (foundFruit == null) {
					System.out.println("Sorry, the fruit " + choice + " is not in stock.");
				}
				else {
					...
					if (ordered == -1) {
						System.out.println("Sorry! The fruit might be OUT-OF-STOCK or your chosen weight is too much. Please restart your order and choose a lower amount.");
						System.out.println("");
					}
					else{
						...
						System.out.println("");
						if (weightChoice == 0) {
							System.out.println("Weight: " + weightChoice + " lbs (you did not order any fruit)");
						}
						else {
						...
						System.out.println("");
						System.out.format("Total cost (including $%5.2f tax): $%5.2f", justTax, plusTax);
						System.out.println("");
						System.out.println("");
						System.out.println("");
					}
	...
	public void showFruits() {
		System.out.println("");
		System.out.println("                    Name  Weight  Price (Per Lb)");
	...
	private Fruit find(String target) {
		 int firstIndex = 0;
	     ...
	     int middleIndex = 0;
	      ...
	      while (firstIndex <= lastIndex) {
	             ...
	             if (fruits[middleIndex].getName().equalsIgnoreCase(target)) {     
	                  return fruits[middleIndex];
	             }
	             else if (fruits[middleIndex].getName().compareToIgnoreCase(target) > 0) {    
